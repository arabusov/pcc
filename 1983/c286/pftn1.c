...
defid( q, class )  NODE *q; {
	...
	if( q == NIL ) return(-1);  /* an error was detected */
...
# endif
	...
	case USTATIC:
		if( scl==STATIC || scl==USTATIC ) return(0);
		break;
	...
	case TYPEDEF:
		if( scl == class ) return(0);
		break;

	case UFORTRAN:
		if( scl == UFORTRAN || scl == FORTRAN ) return(0);
		break;
	...
	case ENAME:
		if( scl != class ) break;
		if( dimtab[p->sizoff] == 0 ) return(0);  /* previous entry just a mention */
		break;

	case ULABEL:
		if( scl == LABEL || scl == ULABEL ) return(0);
	case PARAM:
	case AUTO:
	case REGISTER:
		;  /* mismatch.. */
