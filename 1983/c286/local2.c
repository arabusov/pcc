...
zzzcode( p, c ) NODE *p; {
	...
	/* kludge for long pointers , uses lds or les */
	case 'b':
		np = getlr(p, '1');		/* get temp name */
		if(np->tn.rval == SI)
			expand(p, RNOP, "\tlds\tsi,AR\n");
		else if(np->tn.rval == DI)
			expand(p, RNOP, "\tles\tdi,AR\n");
		else werror(" illegal breg ");
		return;
	...
	case 'S':  /* structure assignment */
		{
			...
			expand(r, FOREFF,"	mov	si,AR\n");
			if (ISPTR(l->in.type))
				expand(l, FOREFF,"	mov	di,AL\n");
			else
			...
			expand(r, FOREFF, "	mov	si,AR\n");
			if (ISPTR(l->in.type))
				expand(l, FOREFF,"	mov	bx,AL\n");
			else
				expand( l, FOREFF, "	lea	bx,AL\n" );
			printf("\tmov\tcx,#%d\n",size);
printf("\tmovb\tax,(si)\n\tmovb\t(bx),ax\n\tinc\tbx\n\tinc\tsi\n\tloop\t.-6\n");
			if (!(rstatus[SI] & STAREG)) printf("	pop	si\n");
#endif
...
void
upput( p, offset ) register NODE *p; {
	...
	case REG:
		if(p->tn.rval == SI) printf("ds");
		else if(p->tn.rval == DI) printf("es");
		else printf( "%s", rnames[p->tn.rval+1] );
		break;
	...
	case OREG:
		...
		if( p->tn.lval != 0 || p->in.name[0] != '\0' ) acon( p, 0 );
		printf( "(%s)", rnames[p->tn.rval] );
		break;
...
void
acon( p, flag ) register NODE *p; { /* print out a constant */
