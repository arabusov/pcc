...
# endif
	...
	case REG:
		...
		*/
		mask = isbreg( p->tn.rval ) ? SAREG|SBREG : SAREG;
		if( istreg( p->tn.rval ) && busy[p->tn.rval]<=1 ) mask |= mask==SAREG ? STAREG : STBREG|STAREG;
		return( shape & mask );
...
expand( p, cookie, cp ) register NODE *p;  register char *cp; {
	...
	for( ; *cp; ++cp ){
		...
		case 'U': /* for upper parts of address, only */
			upput( getlr( p, *++cp ), 1);
			continue;
